...
def _infer_route_from_query(query: str) -> dict:
    """Extract MBTA route_id from a query string for fast-path alerts routing."""
    import re as _re
    q = query.lower()
    if   "red"    in q: return {"route_id": "Red"}
    elif "orange" in q: return {"route_id": "Orange"}
    elif "blue"   in q: return {"route_id": "Blue"}
    elif "green"  in q: return {"route_id": "Green"}
    elif "silver" in q: return {"route_id": "Silver"}
    return {}  # no route_id → mbta_get_alerts returns all alerts
    ...
    with tracer.start_as_current_span("classify_route_and_select_tool") as span:
        ...
        if force_protocol == "mcp":
            ...
            try:
                ...
                decision["thinking_text"] = f'Query: "{query}"\n\nManual override: MCP protocol forced by user.\nUsing direct tool call.\n\nDECISION: USE_MCP'
            ...
            return {
                ...
                "llm_calls": 0,
                "manual_override": True,
                "thinking_text": f'Query: "{query}"\n\nManual override: A2A protocol forced by user.\nCoordinating sub-agents.\n\nDECISION: USE_A2A'
            }
        ...
        if is_greeting_or_simple_query(query):
            with tracer.start_as_current_span("shortcut_path_detection") as shortcut_span:
                ...
                decision = {
                    ...
                    "thinking_text": f'Query: "{query}"\n\nThis is a simple greeting or short query.\nNo routing needed — instant response.\n\nDECISION: SHORTCUT'
                }
        ...
        # ── Fast-path pattern matching (no LLM needed for obvious intents) ──────
        import re as _routing_re
        query_lower_strip = query.lower().strip()

        # ① FARES first — must beat trip-planning pattern ("from Boston to Lowell" in fare queries)
        FARES_TRIGGERS = [
            "fare", "fares", "price", "prices", "cost", "how much",
            "ticket", "tickets", "pay", "payment", "charliecard", "charlie card",
            "monthly pass", "pass", "accessibility", "wheelchair", "accessible",
            "the ride", "paratransit", "disabled", "senior", "reduced fare",
            "commuter rail", "commuter rail ticket", "ferry fare", "bus fare",
            "subway fare", "mbta history", "founded", "oldest subway",
        ]
        is_fares = any(t in query_lower_strip for t in FARES_TRIGGERS)
        if is_fares:
            logger.info("⚡ FAST-PATH: fares/accessibility detected via pattern matching")
            return {
                "path": "a2a",
                "intent": "fares",
                "confidence": 0.92,
                "reasoning": "Fast-path: fare/accessibility query routed to MBTA Fares agent (Frankfurt)",
                "complexity": 0.3,
                "llm_calls": 0,
                "manual_override": False,
                "thinking_text": (
                    f'Query: "{query}"\n\nIntent detected: fares\nConfidence: 92%\n\n'
                    "Reasoning: Fare/accessibility question routed to MBTA Fares specialist agent.\n\n"
                    "DECISION: USE_A2A"
                ),
            }

        # ② Trip planning: "from X to Y", "get from X to Y", "i wanna go from X to Y", etc.
        TRIP_RE = _routing_re.compile(
            r'\bfrom\s+\S.{1,40}?\s+to\s+\S.{1,40}',
            _routing_re.IGNORECASE
        )
        TRIP_TRIGGERS = [
            "how do i get", "how can i get", "how to get",
            "i want to go", "i wanna go", "i need to go",
            "get from", "go from", "travel from", "commute from",
            "route from", "directions from", "way from",
            "take me from", "best way from",
        ]
        is_trip = (
            TRIP_RE.search(query) is not None
            or any(t in query_lower_strip for t in TRIP_TRIGGERS)
        )
        if is_trip:
            logger.info("⚡ FAST-PATH: trip_planning detected via pattern matching")
            return {
                "path": "a2a",
                "intent": "trip_planning",
                "confidence": 0.95,
                "reasoning": "Fast-path: trip planning detected via pattern matching",
                "complexity": 0.5,
                "llm_calls": 0,
                "manual_override": False,
                "thinking_text": (
                    f'Query: "{query}"\n\nIntent detected: trip_planning\nConfidence: 95%\n\n'
                    "Reasoning: Trip planning requires multi-agent coordination to provide optimal route options.\n\n"
                    "DECISION: USE_A2A"
                ),
            }

        # ③ Alerts fast-path: "any delays on X line?", "alerts on orange line", etc.
        ALERTS_RE = _routing_re.compile(
            r'\b(alert|delay|issue|disruption|service|status|problem|running|suspend|cancel|shuttle)'
            r'.{0,40}'
            r'\b(red|orange|blue|green|silver|commuter|mbta|t\b|line|train|subway|bus)',
            _routing_re.IGNORECASE
        )
        ALERTS_TRIGGERS = [
            "any delays", "any issues", "any alerts", "any problems",
            "delays on", "issues with", "status of", "how is the",
            "is the red", "is the orange", "is the blue", "is the green",
            "red line status", "orange line status", "blue line status",
        ]
        is_alerts = (
            ALERTS_RE.search(query) is not None
            or any(t in query_lower_strip for t in ALERTS_TRIGGERS)
        )
        if is_alerts:
            logger.info("⚡ FAST-PATH: alerts detected via pattern matching")
            return {
                "path": "mcp",
                "intent": "alerts",
                "confidence": 0.90,
                "reasoning": "Fast-path: alerts query detected via pattern matching",
                "complexity": 0.2,
                "llm_calls": 0,
                "manual_override": False,
                "mcp_tool": "mbta_get_alerts",
                "mcp_parameters": _infer_route_from_query(query),
                "thinking_text": (
                    f'Query: "{query}"\n\nIntent detected: alerts\nConfidence: 90%\n\n'
...
Analyze and provide routing decision."""
        ...
        try:
            with tracer.start_as_current_span("llm_unified_routing") as llm_span:
                ...
                # Build human-readable thinking text for the UI
                decision["thinking_text"] = (
                    f'Query: "{query}"\n\n'
                    f'Intent detected: {decision.get("intent","unknown")}\n'
                    f'Confidence: {decision.get("confidence",0):.0%}\n\n'
                    f'Reasoning: {decision.get("reasoning","")}\n\n'
                    f'{"DECISION: USE_MCP" if decision["path"]=="mcp" else "DECISION: USE_A2A"}'
                )
...
@agentviz.trace(name="ExchangeAgent", color="#E67E22", role="orchestrator")
async def chat_endpoint(request: ChatRequest):
    ...
    with tracer.start_as_current_span("chat_endpoint") as root_span:
        ...
        metadata = {
            ...
            "thinking_text": decision.get("thinking_text", ""),
        }
        ...
        if chosen_path == "shortcut":
            ...
            # Check if this was actually a shortcut (no agents called)
            if a2a_metadata.get('stategraph_intent') == 'general' and len(a2a_metadata.get('agents_called', [])) == 0:
                path_taken = "shortcut"
                logger.info("   ✓ Reclassified as shortcut (no agents needed)")
            else:
                path_taken = "a2a"
            ...
            # ADD THIS LINE HERE:
            if path_taken == "shortcut":
                metadata["unified_decision"]["path"] = "shortcut"
