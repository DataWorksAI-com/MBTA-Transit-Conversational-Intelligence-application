    ...
    try:
        for doc in agent_registry_col.find():
            ...
            registry["agent_status"][agent_id] = {
                ...
                "tags": doc.get("tags", []),
                "agent_name": doc.get("agent_name", ""),   # ANS URN
            }
...
def _build_agent_payload(agent_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        'agent_name': status_obj.get('agent_name', ''),     # ANS URN
    }
...
def register():
    ...
    tags = data.get('tags', [])
    agent_name = data.get('agent_name', '')     # ANS URN, e.g. urn:agents.dataworksai.com:mbta-transit-ci:alerts
...
# ============================================================================
# ANS — Agent Name Service endpoints (TLD NS + App NS)
# Activated by setting AUTH_NS_URL and ANS_APP environment variables.
# ============================================================================

# Read from environment — zero hardcoded values
_AUTH_NS_URL = os.getenv("AUTH_NS_URL", "")          # e.g. http://96.126.111.107:8300
_ANS_APP = os.getenv("ANS_APP", "mbta-transit-ci")   # application namespace


def _forward_to_auth_ns(label: str, requester_context: dict):
    """
    Forward an agent label to the Generic Auth NS and return its Flask response.

    Uses stdlib urllib so no extra dependency is needed on the registry server.
    """
    ...
    if not _AUTH_NS_URL:
        return jsonify({"error": "AUTH_NS_URL not configured on this server"}), 503

    payload = _json.dumps({"agent": label, "requester_context": requester_context}).encode()
    req = _urlreq.Request(
        f"{_AUTH_NS_URL}/resolve",
        data=payload,
    ...
    except (_urlerr.URLError, _socket.timeout) as exc:
        return jsonify({"error": f"Auth NS unreachable ({_AUTH_NS_URL}): {exc}"}), 503
    except Exception as exc:
        return jsonify({"error": f"Auth NS error: {exc}"}), 503


@app.route('/resolve', methods=['POST'])
def resolve_tld():
    """
    TLD Namespace endpoint.

    POST /resolve
    Body: {"agent_path": "mbta-transit-ci:alerts", "requester_context": {...}}

    Validates the app namespace matches ANS_APP env var, then delegates to
    the App NS resolver (same server, different route).
    """
    ...
    # agent_path format: "<app_namespace>:<label>"
    parts = agent_path.split(":", 1)
    if len(parts) != 2:
        return jsonify({"error": f"invalid agent_path format: {agent_path!r}. Expected '<app>:<label>'"}), 400

    app_ns, label = parts[0].strip(), parts[1].strip()
    if app_ns != _ANS_APP:
        return jsonify({"error": f"unknown app namespace: {app_ns!r}. Expected: {_ANS_APP!r}"}), 404

    if not label:
        return jsonify({"error": "missing agent label in agent_path"}), 400

    return _forward_to_auth_ns(label, requester_context)


@app.route('/resolve/mbta-transit-ci', methods=['POST'])
def resolve_app_ns():
    """
    App Namespace endpoint for the mbta-transit-ci namespace.

    POST /resolve/mbta-transit-ci
    Body: {"agent": "alerts", "requester_context": {...}}
      OR: {"agent_path": "mbta-transit-ci:alerts", "requester_context": {...}}

    Forwards to the Generic Auth NS on the agents server.
    """
    body = request.get_json(silent=True) or {}
    requester_context = body.get("requester_context", {})

    # Accept both "agent" (direct label) and "agent_path" (full path)
    agent_path = body.get("agent_path", body.get("agent", "")).strip()
    if not agent_path:
        return jsonify({"error": "missing agent or agent_path"}), 400

    # Extract label from "mbta-transit-ci:alerts" or plain "alerts"
    label = agent_path.split(":")[-1].strip()
    if not label:
        return jsonify({"error": "could not extract agent label"}), 400
