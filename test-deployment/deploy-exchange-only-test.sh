...
# ============================================================================
# MBTA Exchange + UI TEST - Separate Deployment (No Agents)
# ============================================================================
# Deploys only Exchange Agent + Frontend UI to a separate Linode instance
# Agents must be deployed separately using deploy-agents-only-test.sh
...
if [ -z "$OPENAI_API_KEY" ] || [ -z "$MBTA_API_KEY" ] || [ -z "$AGENTS_IP" ]; then
    ...
    echo ""
    echo "Example:"
    echo "  bash deploy-exchange-only-test.sh \"sk-proj-...\" \"c845...\" \"172.104.25.25\""
    exit 1
fi
...
FIREWALL_LABEL="mbta-exchange-firewall-test"
SSH_KEY_LABEL="mbta-exchange-key-test"
IMAGE_ID="linode/ubuntu22.04"
DEPLOYMENT_ID=$(date +%Y%m%d-%H%M%S)

echo "🚇 MBTA Exchange + UI TEST - Separate Deployment"
...
TARBALL_NAME="mbta-exchange-test-${DEPLOYMENT_ID}.tar.gz"
...
if [ ! -f "${SSH_KEY_LABEL}.pub" ]; then
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_LABEL" -N "" -C "mbta-exchange-test-$DEPLOYMENT_ID" >/dev/null 2>&1
...
INSTANCE_ID=$(linode-cli linodes create \
    ...
    --region "$REGION" \
    --image "$IMAGE_ID" \
    --label "mbta-exchange-test-$DEPLOYMENT_ID" \
    --tags "MBTA-Exchange,test" \
...
# [9/9] Setup services
echo "[9/9] Configuring exchange + frontend (TEST ENVIRONMENT) (5-10 min)..."

ssh -i "$SSH_KEY_LABEL" -o StrictHostKeyChecking=no "root@$PUBLIC_IP" bash << SETUP
set -e

cd /opt
mkdir -p mbta-agentcy-test
cd mbta-agentcy-test
...
# Create .env with TEST marker
cat > .env << ENV
OPENAI_API_KEY=$OPENAI_API_KEY
MBTA_API_KEY=$MBTA_API_KEY
PYTHONPATH=/opt/mbta-agentcy-test
DEPLOYMENT_ENV=test
ENV
...
command=/opt/mbta-agentcy-test/venv/bin/python -m src.exchange_agent.exchange_server
directory=/opt/mbta-agentcy-test
autostart=true
autorestart=true
stderr_logfile=/var/log/mbta-exchange.err.log
stdout_logfile=/var/log/mbta-exchange.out.log
environment=PYTHONPATH="/opt/mbta-agentcy-test",DEPLOYMENT_ENV="test"
S1

# Supervisor config - Frontend UI
cat > /etc/supervisor/conf.d/mbta-frontend.conf << 'S2'
[program:mbta-frontend]
command=/opt/mbta-agentcy-test/venv/bin/python -m src.frontend.chat_server
directory=/opt/mbta-agentcy-test
autostart=true
autorestart=true
...
echo ""
echo "🎉 ============================================================================"
echo "🎉 MBTA Exchange + UI TEST Deployed!"
echo "🎉 ============================================================================"
echo ""
...
cat > exchange-test-deployment-info.txt << EOF
MBTA Exchange + UI TEST Deployment
===================================
Deployed: $(date)
...
echo "💾 Deployment info saved: exchange-test-deployment-info.txt"
echo ""
