...
# Deploy Northeastern Registry v3 TEST with Full Description Support
# Usage: bash deploy_registry_v3_test.sh <MONGODB_URL> [REGION] [INSTANCE_TYPE]
...
if [ -z "$MONGODB_URL" ]; then
    ...
    echo "   Example: bash deploy_registry_v3_test.sh 'mongodb+srv://user:pass@cluster.mongodb.net/'"
    exit 1
fi
...
FIREWALL_LABEL="Northeastern-registry-v3-test"
SSH_KEY_LABEL="Northeastern-registry-v3-key-test"
IMAGE_ID="linode/ubuntu22.04"
DEPLOYMENT_ID=$(date +%Y%m%d-%H%M%S)

echo "🗄️ Deploying Northeastern Registry v3 TEST (Semantic Discovery Ready)"
echo "======================================================================"
...
if [ ! -f "${SSH_KEY_LABEL}.pub" ]; then
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_LABEL" -N "" -C "registry-test-$DEPLOYMENT_ID" >/dev/null 2>&1
fi
echo "✅ SSH key ready"
...
INSTANCE_ID=$(linode-cli linodes create \
    ...
    --region "$REGION" \
    --image "$IMAGE_ID" \
    --label "Northeastern-registry-v3-test-$DEPLOYMENT_ID" \
    --tags "Northeastern-Registry,v3,semantic,test" \
...
cd /home/ubuntu
sudo -u ubuntu mkdir -p Northeastern-registry-test
cd Northeastern-registry-test
...
def register():
    ...
    save_registry()
    print(f"✅ Registered (TEST): {agent_id}")

    return jsonify({"status": "success", "message": f"Agent {agent_id} registered in TEST environment"})
...
if __name__ == '__main__':
    ...
    print(f"🚀 Northeastern Registry v3 TEST on port {port}")
...
def health_check():
    try:
        ...
        return jsonify({"status": "healthy", "mongodb": "connected", "environment": "TEST"})
    except Exception as e:
...
<head>
    ...
    <title>Northeastern Registry v3 TEST - Semantic Discovery</title>
    <style>
...
<body>
    <div class="test-banner">
        🧪 TEST ENVIRONMENT 🧪 - Registry v3 Testing Instance
    </div>
    
    <div class="container">
        <div class="header">
            <h1>🗄️ Northeastern Registry v3 TEST</h1>
            <p class="subtitle">Semantic Agent Discovery Platform - Test Environment</p>
        </div>
...
chown -R ubuntu:ubuntu /home/ubuntu/Northeastern-registry-test

cat > /etc/supervisor/conf.d/registry.conf << SUPEOF
[program:registry]
command=/home/ubuntu/Northeastern-registry-test/.venv/bin/python registry.py
directory=/home/ubuntu/Northeastern-registry-test
user=ubuntu
autostart=true
autorestart=true
...
[program:agentfacts]
command=/home/ubuntu/Northeastern-registry-test/.venv/bin/python agentFactsServer.py
directory=/home/ubuntu/Northeastern-registry-test
user=ubuntu
autostart=true
autorestart=true
...
server {
    ...
    location / {
        root /home/ubuntu/Northeastern-registry-test;
...
chmod 755 /home/ubuntu/Northeastern-registry-test
chmod 644 /home/ubuntu/Northeastern-registry-test/registry-ui.html
...
echo ""
echo "🎉 ================================================================"
echo "🎉 Northeastern Registry v3 TEST Deployed!"
echo "🎉 ================================================================"
echo ""
...
echo "   • MongoDB persistence (TEST database)"
echo "   • Real-time dashboard with TEST banner"
echo "   • RESTful API"
echo ""
...
echo "       \"description\": \"Test agent for development\""
echo "     }'"
echo ""
echo "   curl -X PUT http://$PUBLIC_IP:6900/agents/test-agent/status \\"
echo "     -H 'Content-Type: application/json' \\"
echo "     -d '{"
echo "       \"alive\": true,"
echo "       \"capabilities\": [\"testing\", \"development\"]"
echo "     }'"
echo ""
