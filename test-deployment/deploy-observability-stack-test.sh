...
# ============================================================================
# MBTA Observability Stack TEST - Linode Deployment
# ============================================================================
...
FIREWALL_LABEL="mbta-observability-firewall-test"
SSH_KEY_LABEL="mbta-observability-key-test"
IMAGE_ID="linode/ubuntu22.04"
DEPLOYMENT_ID=$(date +%Y%m%d-%H%M%S)

echo "📊 MBTA Observability Stack TEST Deployment"
...
if [ ! -f "${SSH_KEY_LABEL}.pub" ]; then
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_LABEL" -N "" -C "observability-test-$DEPLOYMENT_ID" >/dev/null 2>&1
...
INSTANCE_ID=$(linode-cli linodes create \
    ...
    --region "$REGION" \
    --image "$IMAGE_ID" \
    --label "mbta-observability-test-$DEPLOYMENT_ID" \
    --tags "MBTA-Observability,test" \
...
echo "[7/8] Deploying observability stack (TEST ENVIRONMENT) (2-3 min)..."

ssh -i "$SSH_KEY_LABEL" -o StrictHostKeyChecking=no "root@$PUBLIC_IP" bash << 'OBSERVABILITY_SETUP'
set -e

# Create directory
mkdir -p /opt/observability-test
cd /opt/observability-test
...
services:
  jaeger:
    ...
    container_name: mbta-jaeger-test
  ...
  otel-collector:
    ...
    container_name: mbta-otel-collector-test
    restart: unless-stopped
    ports:
  ...
  clickhouse:
    ...
    container_name: mbta-clickhouse-test
    restart: unless-stopped
    ports:
  ...
  grafana:
    ...
    container_name: mbta-grafana-test
    restart: unless-stopped
    ports:
...
echo "✅ Observability stack deployed (TEST)"
OBSERVABILITY_SETUP
...
echo ""
echo "🎉 ============================================================================"
echo "🎉 MBTA Observability Stack TEST Deployed!"
echo "🎉 ============================================================================"
echo ""
...
echo "   ssh -i $SSH_KEY_LABEL root@$PUBLIC_IP 'cd /opt/observability-test && docker-compose ps'"
echo "   ssh -i $SSH_KEY_LABEL root@$PUBLIC_IP 'cd /opt/observability-test && docker-compose logs -f'"
echo ""
echo "⚠️  NEXT STEPS:"
echo "   1. Update Test Exchange Server .env:"
echo "      OTEL_ENDPOINT=http://$PUBLIC_IP:4317"
echo ""
echo "   2. Update Test Agents Server .env:"
echo "      OTEL_ENDPOINT=http://$PUBLIC_IP:4317"
echo ""
echo "   3. Restart services on both test servers:"
...
cat > observability-test-deployment-info.txt << EOF
MBTA Observability TEST Deployment
===================================
Deployed: $(date)
...
Update Test Application Servers:
---------------------------------
Add to /opt/mbta-agentcy-test/.env on BOTH servers:
...
echo "💾 Deployment info saved: observability-test-deployment-info.txt"
echo ""
