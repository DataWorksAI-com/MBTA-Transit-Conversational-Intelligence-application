...
# ============================================================================
# MBTA Agents - Separate Deployment TEST (No Registry)
# ============================================================================
...
if [ -z "$MBTA_API_KEY" ] || [ -z "$OPENAI_API_KEY" ]; then
    ...
    echo ""
    echo "Example:"
    echo "  bash deploy-agents-only-test.sh \"c845...\" \"sk-proj-...\""
    exit 1
fi
...
FIREWALL_LABEL="mbta-agents-firewall-test"
SSH_KEY_LABEL="mbta-agents-key-test"
IMAGE_ID="linode/ubuntu22.04"
DEPLOYMENT_ID=$(date +%Y%m%d-%H%M%S)

echo "🚇 MBTA Agents TEST - Separate Deployment"
...
TARBALL_NAME="mbta-agents-test-${DEPLOYMENT_ID}.tar.gz"
...
if [ ! -f "${SSH_KEY_LABEL}.pub" ]; then
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_LABEL" -N "" -C "mbta-agents-test-$DEPLOYMENT_ID" >/dev/null 2>&1
...
INSTANCE_ID=$(linode-cli linodes create \
    ...
    --region "$REGION" \
    --image "$IMAGE_ID" \
    --label "mbta-agents-test-$DEPLOYMENT_ID" \
    --tags "MBTA-Agents,test" \
...
# [9/9] Setup services
echo "[9/9] Configuring agents (TEST ENVIRONMENT) (3-5 min)..."

ssh -i "$SSH_KEY_LABEL" -o StrictHostKeyChecking=no "root@$PUBLIC_IP" bash << SETUP
set -e

cd /opt
mkdir -p mbta-agents-test
cd mbta-agents-test
...
PYTHONPATH=/opt/mbta-agents-test
DEPLOYMENT_ENV=test
ENV
...
[program:mbta-alerts]
command=/opt/mbta-agents-test/venv/bin/python -m agents.alerts.main
directory=/opt/mbta-agents-test
autostart=true
autorestart=true
stderr_logfile=/var/log/mbta-alerts.err.log
stdout_logfile=/var/log/mbta-alerts.out.log
environment=PYTHONPATH="/opt/mbta-agents-test",DEPLOYMENT_ENV="test"
S1

# Supervisor config - Planner Agent
cat > /etc/supervisor/conf.d/mbta-planner.conf << 'S2'
[program:mbta-planner]
command=/opt/mbta-agents-test/venv/bin/python -m agents.planner.main
directory=/opt/mbta-agents-test
autostart=true
autorestart=true
...
command=/opt/mbta-agents-test/venv/bin/python -m agents.stopfinder.main
directory=/opt/mbta-agents-test
autostart=true
autorestart=true
...
echo ""
echo "🎉 ============================================================================"
echo "🎉 MBTA Agents TEST Deployed!"
echo "🎉 ============================================================================"
echo ""
...
echo ""
echo "⚠️  NEXT STEP: Update test exchange code with this IP!"
...
cat > agents-test-deployment-info.txt << EOF
MBTA Agents TEST Deployment
===========================
Deployed: $(date)
...
echo "💾 Deployment info saved: agents-test-deployment-info.txt"
echo ""
