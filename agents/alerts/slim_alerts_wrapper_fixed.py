...
class AlertsExecutor(AgentExecutor):
    ...
    def is_historical_question(self, query: str) -> bool:
        ...
        historical_indicators = [
            ...
            "based on past", "on average", "generally",
            "average delay", "typical delay", "how long will",
            "past data", "past incidents", "past delays", "delay pattern",
            "long does it take", "take to fix", "take to clear", "take to resolve"
        ]
        # Note: "historical" intentionally excluded — it appears in injected trip context
        # and would falsely trigger historical mode for trip planning queries
    ...
    @agentviz.trace(name="CrowdingCheck", color="#F39C12")
    ...
    @agentviz.trace(name="FetchAlerts", color="#E74C3C")
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
        ...
        try:
            ...
            # Only check the user's original words — strip injected orchestrator context
            # (everything from "Trip context:", "Should I wait", or "ALERTS CONTEXT:" onward)
            # ================================================================
            user_words = message_text
            for marker in ["Trip context:", "ALERTS CONTEXT:", "Should I wait for any"]:
                if marker in user_words:
                    user_words = user_words[:user_words.index(marker)].strip()
                    break

            if self.is_historical_question(user_words):
                logger.info("📚 Historical question")
                response_text = self.answer_historical_question(user_words)
            ...
            wants_prediction = any(kw in query_lower for kw in [
                ...
                "trip context", "alternative route", "estimated delay duration",
                "wait for any delays", "take an alternative"
            ])
