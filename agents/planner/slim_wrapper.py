...
class PlannerExecutor(AgentExecutor):
    ...
    def detect_multiple_routes_request(self, query: str) -> bool:
        ...
        keywords = [
            ...
            "different route", "alternate route", "alternative route",
            "options", "2 route", "3 route", "2 options", "3 options"
        ]
        # Also catch "give me 2", "show 2 routes", "2 ways", "3 ways" etc.
        num_pattern = _re.search(r"\b[2-5]\s*(route|option|way|path|alternative)", q)
        detected = any(kw in q for kw in keywords) or bool(num_pattern)
        if detected:
...
Response:"""
    ...
    # Common Boston landmark aliases -> MBTA station names
    LOCATION_ALIASES = {
        "harbor": "aquarium",
        "boston harbor": "aquarium",
        "long wharf": "aquarium",
        ...
        "children's museum": "south station",
        "convention center": "world trade center",
        "bcec": "world trade center",
    ...
    async def find_stop_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Apply location aliases (harbor -> aquarium, fenway -> kenmore, etc.)
            name_clean = self.LOCATION_ALIASES.get(name_clean, name_clean)
    ...
    @agentviz.trace(name="PlannerAgent", color="#3498DB")
...
def main():
    ...
    agent_card = AgentCard(
        ...
        url=f"http://{os.getenv('PUBLIC_IP', os.getenv('AGENT_HOST', 'localhost'))}:{os.getenv('AGENT_PORT', '50052')}/",
        version="5.1.0",
    ...
    # Add /health endpoint for DANS health checks
    from starlette.requests import Request
    from starlette.responses import JSONResponse
    from starlette.routing import Route

    async def _health(request: Request) -> JSONResponse:
        return JSONResponse({"ok": True, "status": "healthy", "service": "mbta-planner-agent"})

    app.routes.append(Route("/health", endpoint=_health, methods=["GET"]))
