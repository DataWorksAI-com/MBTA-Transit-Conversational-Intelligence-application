...
class ResolutionCache:
    ...
    def __init__(self):
        ...
        # _agent_index: agent_name → set(cache_keys) 
        # Allows O(1) lookup for invalidation
    ...
    async def _remove_key(self, key: str, agent_name: str):
        """Internal helper to remove from both store and index safely."""
        if key in self._store:
            del self._store[key]
        
        if agent_name in self._agent_index:
            self._agent_index[agent_name].discard(key)
            # Clean up the set if it's empty to save memory
    ...
    async def set(self, agent_name: str, context: Any, data: Dict, ttl: int) -> None:
        """Store resolution data under (agent_name, context) with a secondary index."""
        key = self._make_key(agent_name, context or {})
        now = datetime.now()
        async with self._lock:
            self._store[key] = {
    ...
    async def invalidate(self, agent_name: str) -> bool:
        """High-performance invalidation using the secondary index."""
        async with self._lock:
            # We copy the set to avoid "size changed during iteration" errors
