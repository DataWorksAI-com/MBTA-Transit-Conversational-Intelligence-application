"""
Authoritative Nameserver for the Planner agent.

Owns:  urn:agents.local:mbta-transit-ci:planner
Port:  8301
"""
import sys
import os
...
app = FastAPI(title="planner-auth-ns", version="2.0.0")
    ...
    "servers": [
        {
            ...
            "http_endpoint": "http://localhost:8002",
...
async def resolve(req: AuthResolutionRequest) -> AuthResolutionResponse:
    ...
    if not ranked:
        raise HTTPException(status_code=503, detail="All planner servers are currently unhealthy")
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8301, log_level="info")
